...
- **Context Discipline**: Always redirect heavy test/build output to `/tmp/` to prevent context explosion:
  `pytest > /tmp/test.log 2>&1 || tail -25 /tmp/test.log`
- **Deterministic Code Pipeline (Multi-Agent TDD)**:
  ...
  - **CRITICAL - Dynamic QA / Dev Model Assignment (No Hardcoding)**:
    1. The Main Model MUST NOT assume or hardcode which model does QA and which model does Dev.
    2. If the user's prompt **does NOT** explicitly include which model is for QA and which is for Dev:
       The Main Model **MUST STOP AND ASK THE USER** to confirm the model assignment. Check `ask --status` to show available CLIs/models (e.g. Antigravity/agy, Claude, Codex, DeepSeek) to assist the user.
    3. If the user's prompt **specifies** the models (e.g. "QA là Antigravity, Dev là DeepSeek"):
       Extract them and pass explicitly: `--qa <qa_model> --dev <dev_model>` (e.g. `--qa agy --dev deepseek`).
- **Guardrails**: Auto-resume catches 429 rate-limits and context exhaustion automatically up to 3 times.
