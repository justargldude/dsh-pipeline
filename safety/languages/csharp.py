"""C# language driver for AST safety analysis (anti-reward-hacking v2.3 Stage 1).

Owns every C#-specific concern previously hardcoded inside
`safety/ast_guard.py`: symbol extraction, assert predicate extraction,
...
class CSharpDriver(BaseLanguageDriver):
    ...
    @property
    def language(self):
        # Shared cached Language instance (Opt 8.3): identical object across
        # all CSharpDriver instances and the SymbolExtractor.
        return get_csharp_language()

    @property
    def parser(self):
        # Parser stays per-instance (lightweight) — see tests/red_batch3 Opt 8.3.
        # Implemented as a property constructing a fresh Parser each access,
        # matching the legacy ASTGuard contract where each guard owns one.
    ...
    def extract_symbols(self, root, code_bytes: bytes) -> List[CSharpSymbol]:
    ...
    def validate_transition(
        ...
        """Comprehensive AST validation across C# code transitions.

        Enforces (unchanged from the pre-refactor ASTGuard logic):
        1. Valid C# syntax without introduced parse errors.
        2. Disallowed deletion of classes, interfaces, structs, or methods.
        3. Target symbol boundary enforcement.
        4. Overload distinction and precision.
        5. Assertion semantic validation (rejects weakening/removal/tautologies).
        6. Early return & dummy stub bypass detection.
        """
        old_bytes = old_code.encode("utf-8")
        new_bytes = new_code.encode("utf-8")

        # Snapshot one stable Parser for the whole validation pass (matches
        # the legacy ASTGuard contract of a single per-instance parser).
        parser = self.parser
        ...
        old_symbols = self.extract_symbols(old_tree.root_node, old_bytes)
        new_symbols = self.extract_symbols(new_tree.root_node, new_bytes)
        ...
        old_asserts = self.extract_assert_predicates(old_tree.root_node, old_bytes)
        new_asserts = self.extract_assert_predicates(new_tree.root_node, new_bytes)
