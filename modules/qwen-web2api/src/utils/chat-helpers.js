...
/**
 * 从消息内容项中识别媒体类型和数据源
 * 支持 OpenAI 兼容格式的图片/视频/音频内容
 * @param {object} item - 消息内容项
 * @returns {{ data: string|null, mediaType: string, defaultExt: string }|null}
 */
const extractMediaInfo = (item) => {
    ...
    // 图片类型
    if (type === 'image_url') {
        return { data: item.image_url?.url, mediaType: 'image', defaultExt: 'png' };
    }
    if (type === 'image' && (typeof item.image === 'string')) {
        return { data: item.image, mediaType: 'image', defaultExt: 'png' };
    }

    // 视频类型
    if (type === 'video_url') {
        return { data: item.video_url?.url, mediaType: 'video', defaultExt: 'mp4' };
    }
    if (type === 'video' && (typeof item.video === 'string')) {
        return { data: item.video, mediaType: 'video', defaultExt: 'mp4' };
    }

    // 音频类型 — OpenAI 官方 input_audio 格式
    if (type === 'input_audio') {
        const format = item.input_audio?.format || 'wav';
        const rawData = item.input_audio?.data;
        // input_audio.data 是纯 base64（无 data: 前缀），需要补全
        const data = rawData ? `data:audio/${format};base64,${rawData}` : null;
        return { data, mediaType: 'audio', defaultExt: format };
    }
    if (type === 'audio_url') {
        return { data: item.audio_url?.url, mediaType: 'audio', defaultExt: 'wav' };
    }
    if (type === 'audio' && (typeof item.audio === 'string')) {
        return { data: item.audio, mediaType: 'audio', defaultExt: 'wav' };
    }
...
/**
 * 处理媒体内容的上传（支持 base64 和 URL 直传）
 * @param {string} rawData - base64 数据（带 data: 前缀）或 HTTP(S) URL
 * @param {string} mediaType - 媒体类型 ('image'|'video'|'audio')
 * @param {string} defaultExt - 默认文件扩展名
 * @param {object} cacheManager - 文件缓存管理器
 * @returns {Promise<{type: string, [key: string]: string}|null>} 上游消息内容项
 */
const processMediaUpload = async (rawData, mediaType, defaultExt, cacheManager) => {
    if (!rawData) return null;

    // HTTP(S) URL 直传，不需要上传到 OSS
    if (rawData.startsWith('http://') || rawData.startsWith('https://')) {
        logger.info(`${mediaType} 使用URL直传: ${rawData.substring(0, 80)}...`, 'UPLOAD');
        return { type: mediaType, [mediaType]: rawData };
    }

    // 处理 base64 数据
    const regex = /data:(.+);base64,/;
    const fileTypeMatch = rawData.match(regex);
    const fileExtension = fileTypeMatch && fileTypeMatch[1]
        ? fileTypeMatch[1].split('/')[1] || defaultExt
        : defaultExt;
    const filename = `${generateUUID()}.${fileExtension}`;
    const base64Data = rawData.replace(regex, '');
    const signature = sha256Encrypt(base64Data);

    try {
        const buffer = Buffer.from(base64Data, 'base64');
        const cacheHit = cacheManager.cacheIsExist(signature);

        if (cacheHit) {
            logger.info(`${mediaType} 命中缓存`, 'UPLOAD');
            return { type: mediaType, [mediaType]: cacheManager.getCache(signature).url };
        }

        const uploadResult = await uploadFileToQwenOss(buffer, filename, accountManager.getAccountToken());
        if (uploadResult && uploadResult.status === 200) {
            cacheManager.addCache(signature, uploadResult.file_url);
            logger.info(`${mediaType} 上传成功: ${uploadResult.file_url}`, 'UPLOAD');
            return { type: mediaType, [mediaType]: uploadResult.file_url };
        }
    } catch (error) {
        logger.error(`${mediaType}上传失败`, 'UPLOAD', '', error);
    }
                ...
                if (item.type === 'text') {
                    ...
                    if (mediaInfo && mediaInfo.data) {
                        const result = await processMediaUpload(
                            mediaInfo.data, mediaInfo.mediaType, mediaInfo.defaultExt, imgCacheManager
                        );
                        if (result) {
